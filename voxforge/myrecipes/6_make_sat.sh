...
time ( ./steps/train_sat.sh --cmd "$train_cmd" \  #config
	$leaves $gauss \                       #hiperparam
	data/train data/lang ${dir_ali} \      #input
	${dir_exp}/model ) 2> train.time                       #output

./utils/mkgraph.sh data/lang_test ${dir_exp}/model ${dir_exp}/graph

time ( ./steps/decode_fmllr.sh --config conf/decode.config --nj $njobs --cmd "$decode_cmd" \
